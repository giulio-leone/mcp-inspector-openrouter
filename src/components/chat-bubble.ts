...
export class ChatBubble extends BaseElement {
  ...
  protected override render(): unknown {
    ...
    switch (this.role) {
      ...
      case 'tool_call':
        return html`<div class="bubble-body"><span class="tool-icon">${unsafeHTML(ICONS.zap)}</span> <strong>Running action:</strong> ${this.toolName} <code>${JSON.stringify(this.toolArgs)}</code></div><div class="bubble-time">${time}</div>${this.editable ? html`<div class="bubble-actions"><button class="bubble-action-btn" title="Delete this message and everything after it" @click=${this._handleDelete}>${unsafeHTML(ICONS.trash)}</button></div>` : nothing}`;

      case 'tool_result':
        return html`<div class="bubble-body"><span class="tool-icon">${unsafeHTML(ICONS.checkCircle)}</span> <strong>Action finished:</strong> ${this.toolName} → <code>${this.content}</code></div><div class="bubble-time">${time}</div>${this.editable ? html`<div class="bubble-actions"><button class="bubble-action-btn" title="Delete this message and everything after it" @click=${this._handleDelete}>${unsafeHTML(ICONS.trash)}</button></div>` : nothing}`;

      case 'tool_error':
        return html`<div class="bubble-body"><span class="tool-icon">${unsafeHTML(ICONS.xCircle)}</span> <strong>Action needs attention:</strong> ${this.toolName} → <code>${this.content}</code></div><div class="bubble-time">${time}</div>${this.editable ? html`<div class="bubble-actions"><button class="bubble-action-btn" title="Delete this message and everything after it" @click=${this._handleDelete}>${unsafeHTML(ICONS.trash)}</button></div>` : nothing}`;
